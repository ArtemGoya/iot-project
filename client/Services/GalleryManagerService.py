...
class GelleryManagerService():
    ...
    def __init__(self):
        ...
        self.__min_temperatura_zewn = 0 # minimalna temperatura zewn przy jakiej można otwierać okna
        self.__temperatura_docelowa = 21 # docelowa temperatura utrzymywana w pomieszczeniu
    ...
    def set_tryb_swiatel(self, value):
        if value == Swiatla.OTWARTE:
            set_light(wlaczone=True)
            self.__swiatla_wlaczone = True
        elif value == Swiatla.ZAMKNIETE:
            set_windows(wlaczone=False)
            self.__swiatla_wlaczone = False
    ...
    def loop(self):
        ...
        if self.__tryb_okien == Okna.AUTO:
            ...
            if self.__okna_otwarte == True and (                                # gdy okna są otwarte i
                    self.__max_wiatr < out_wiatr                                # wiatr jest za silny
                    or self.__min_temperatura_zewn < out_temperatura            # lub temperatura zewn jest za niska
                    or self.__temperatura_docelowa > in_temperatura             # temperatura wewn jest niższa od docelowej
                    and out_temperatura < in_temperatura                        # i temperatura na zewnątrz jest niższa niż w środku
                    or self.__wilg_docelowa > in_wilgotnosc                     # to samo dla wilgotności
                    and out_wilgotnosc < in_wilgotnosc
                    ):
                set_windows(otwarte=False)
                self.__okna_otwarte = False
            elif self.__okna_otwarte == False:                                  # w innym wypadku staramy się trzymać okna otwarte
                set_light(wlaczone=True)
                self.__swiatla_wlaczone = True
